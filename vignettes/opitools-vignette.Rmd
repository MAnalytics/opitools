...
abstract: The development of `'opitools'` is instigated by the lack of tools for analyzing how a theme or subject within a text document may have impacted the overall opinion expressed by a given text document. The package functions can be grouped into two, namely (1) exploratory functions - for exploring the characteristics of word (terms) in a document, such as the distribution and word importance, and (2) impact analytical functions - for carrying out the impact analysis. In this vignette, the utility of each of the functions contained in the package is demonstrated, with applications to real-life datasets. The outcomes of the demonstrations show that `opitools` can help a user in identifying the most important issues or subjects that may be driving the opinions expressed in a text document.
...
The development of `'opitools'` is instigated by the lack of tools for analyzing the extent to which a selected theme or subject from a digital text document (DTD) may have impacted the overall opinion expressed in the DTD. The functions in the package can be categorized into two groups, namely (1) exploratory functions - for exploring the characteristics of words (terms) in a document, e.g. the distribution and the words importance, and (2) impact analytical functions - for carrying out the impact analysis of a document. In this vignette, the potentials of `opitools` for considerably wide applications is demonstrated by applying the package to a number of significantly different subject areas, such as the law enforcement [@Adepeju2021], politics, entertainment and marketing. 
...
In order to demonstrate the potentials of `opitools` for application in a wide variety of domains, the following example datasets are presented:

```{r, echo=FALSE, include=FALSE}
col1 <- c("1", "2", "3")
col2 <- c("`policing_otd`","`reviews_otd`","`debate_otd`")
col3 <- c("`Law Enforcement`","`Transport`", "`Politics`")
col4 <- c("A text document (an OTD) containing twitter posts on police/policing", "A text document (an OTD) containing the  customer reviews of the Piccadilly train station (Manchester) downloaded from the www.tripadvisor.co.uk'. The reviews cover from July 2016 to March 2021.","...")
col5 <- c("Twitter.com", "www.tripadvisor.co.uk","https://www.youtube.com/watch?v=855Am6ovK7s")
...
The `policing_otd` includes individual tweets concerning neighbourhood policing within a defined geographical area. The `reviews_otd` includes the reviews of Piccadilly train station in Manchester (UK) by the customers. The reviews covers July 2016 to March 2021. Lastly, the `debate_otd` contains individual comments on the first US presidential debate between Hillary Clinton and Donald Trump in September 2016.   


# Exploration of a text document

Table 2 shows the list of exploratory functions, while Table 3 shows the list of impact analytical functions in the `opitools` package.
