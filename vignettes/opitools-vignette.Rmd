...
abstract: The development of `'opitools'` is inspired by the lack of tools for performing opinion impact analysis of a digital text document (DTD). The package includes a number of interrelated functions for exploring and analyzing text records in order to complete an opinion impact analysis of a digital text document (DTD). The utility of `Opitools` is demonstrated with examples from the law enforcement, marketing and politics.
...
Table 2 shows two exploratory functions embedded in `'opitools'`, namely `'word_distrib'` and `'word_imp'`. Details as follow:
...
The second exploratory function `'word_imp'` can be used to highlight the level of importance of each word in a DTD. The highlighted words can then be used to identify different themes (or subjects) that are inherent within or related to the DTD in question. Two metrics, namely (i) `'term frequency (tf)'` and `term frequency inverse document frequency (tf-idf)` [@Silge2016] are provided in the function. In Figure 2, results 1, 2 and 3 are generated from the function using the datasets; `policing_dtd`, `reviews_dtd` and `debate_dtd`, respectively. The results setting the parameter `metric` as `'tf'` and `'tf_idf'` are labeled as A and B, respectively. The function draws from `wordcloud2` package in R [@Dawei2018], in order to render the graphics as shown in Figure 2.
...
In Figure `r fig$ref("figs2")`, the size of a word represents its level of importance according to the selected metric (i.e. 'tf' or 'tf-idf'). From each graphic, a user can easily identify related words that denote certain themes or subjects within the document. In Figure 1A, words such as lockdown, infect and covid, refers to the pandemic-related sentiment under which the policing were operating as at the time the tweets were posted. Does the pandemic has significant impacts on the public opinion of the police activities? Figure 1B shows that the word importance may vary significantly by the metric. In Figure 2A, we see that related words, such as restaurant, shops, food and coffee, denoting refreshment outlets, are important to the customers. Do these refreshment outlets impact the overall customers' opinion of the train services?. Similarly, related words such as board, map and display, denoting signages may have influenced customers opinion of the station?.
...
Table 4 summarizes the results of impact analysis using the example datasets in Table 1 for different research questions (RQs). The application in law enforcement (as conducted above) is entered as the first record of the table.  For demonstration, we employed the same score function (`metric = 1`, i.e. `polarity score = (P - N)/(P + N)*100`) in each application, where P and N represent positive and negative sentiments, respectively. See the documentation for details.
...
In the marketing application (2a & 2b), we tested the impacts of two themes, namely; refreshment outlets and the signages, on the customers' opinions of the train services. The statistics (S=67.92, pvalue = 0.01) show an overall positive customers' opinions of the train station or services, with the refreshments outlets in and around the station contributing significantly towards the positive opinions. On the other hand, the result (2b) indicates that the signages in and around the station have no significant influence (p=0.1) on the positive review of the train service. The last application attempts to identify factors that may have contributed to viewers opinion of a political debate. The statistics (S=-0.33, pvalue = 0.93) show that viewers have a negative opinion on the debate, but their views on the democratic candidate (Hillary Clinton) is not a factor that contributes significantly to the overall negative opinion of the debate.

### Using a user-defined opinion score function

An pinion score function may be defined in a variety of ways depending on the application domain. For instance, [@Razorfish2019] defines opinion score of a product brand as `score = (P + O - N)/(P + O + N)`, where `P`, `O`, and `N`, represent the amount/proportion of positive, neutral and negative, sentiments, respectively. Thus, a user ought to be able to plug in their own opinion score function into the `opi_impact` function. Employing the stated example function, we can plug in the function and re-run the analysis as follows: 
...
The `opitools` package has been developed in order to address the lack of mechanism for carrying out impact analysis of opinions for digital text document (DTD). The utility of the package was originally demonstrated in [@Adepeju2021] in its application in law enforcement. domain. Further, this vignette demonstrates the potentials of the package for application across a wide variety of areas, including marketing and politics. This package is being updated on a regular basis to add more functionalities. 
